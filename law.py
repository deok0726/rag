...
def retrieve(context, question, index_name):
    pc = Pinecone(api_key=pinecone_api_key)
    index = pc.Index(index_name)

    model = SentenceTransformer('jhgan/ko-sroberta-multitask')
    query_vector = model.encode(question).tolist()
    result = index.query(namespace="law", vector=[query_vector], top_k=3, include_metadata=True)
...
if __name__ == "__main__":
    ...
    cmd = sys.argv[1:]
    if cmd[0] == 'u':
        folder = '/svc/project/genaipilot/rag/data/law/terms/'
        ...
        # '''
        for i in range(5):
            retrieve(context[i], question[i], 'law')
    elif cmd[0] == 't':
